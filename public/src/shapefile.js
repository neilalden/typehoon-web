...
function ShpHeader(src) {
	if (src.getLength() < 100) alert("Not a valid shape file header (too small)");

	if (src.getSLong() != 9994) alert("Not a valid signature. Expected 9994");
...
function ShpRecord(src) {
	...
	if (availableBytes == 0) throw new ShpError("No Data", ShpError.ERROR_NODATA);
	...
	switch (this.shapeType) {
		...
		case ShpType.SHAPE_POLYLINEM:
			throw new ShpError(
				this.shapeType + " Shape type is currently unsupported by this library",
			);
			break;
		default:
			throw new ShpError(
				"Encountered unknown shape type (" + this.shapeType + ")",
			);
			break;
	}
}
